...
func play(textToDisplayPrimary:String, textToDisplaySecondary:String):
	...
	tween.interpolate_property(title,"percent_visible",0,1,8,Tween.TRANS_LINEAR)
	tween.interpolate_property(title,"modulate:a",0,.80,8,Tween.TRANS_LINEAR)
	tween.start()


func _on_Tween_tween_completed(object, key):
	if object == title and key == ":percent_visible":
		tween.interpolate_property(heading,"percent_visible",0,1,8,Tween.TRANS_LINEAR)
		tween.interpolate_property(heading,"modulate:a",0,.80,8,Tween.TRANS_LINEAR)
		tween.start()
